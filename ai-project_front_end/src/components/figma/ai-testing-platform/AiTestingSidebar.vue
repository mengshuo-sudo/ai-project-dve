...
const localStorageKey = 'ai-testing-platform.sidebar.collapsed'
const expandedWidth = 224
const collapsedWidth = 72
const isGroupOpen = ref({
  asset: true,
  execution: true
})
...
function groupKeyFromLabel(label: string) {
  if (label === '资产中心') return 'asset'
  if (label === '执行中心') return 'execution'
  return null
}

function isGroupExpanded(label: string) {
  const key = groupKeyFromLabel(label)
  if (!key) return true
  return Boolean((isGroupOpen.value as Record<string, boolean>)[key])
}

function setGroupExpanded(label: string, next: boolean) {
  const key = groupKeyFromLabel(label)
  if (!key) return
  isGroupOpen.value = { ...isGroupOpen.value, [key]: next }
  try {
    localStorage.setItem(`${localStorageKey}.group.${key}.open`, next ? '1' : '0')
  } catch {}
}

function toggleGroupExpanded(label: string) {
  if (isCollapsed.value) return
  setGroupExpanded(label, !isGroupExpanded(label))
}

function groupChildrenStyle(label: string) {
  const open = isGroupExpanded(label)
  const maxHeight = label === '执行中心' ? 120 : 220
  return {
...
function isExtraLinkActive(item: LinkItem) {
  if (!item.to) return false
  const to = item.to
  if (to.endsWith('/reports')) {
    return route.path === to
  }
  return route.path === to || route.path.startsWith(`${to}/`)
}

function isSettingsLinkActive(item: LinkItem) {
  if (!item.to) return false
  const to = item.to
  return route.path === to || route.path.startsWith(`${to}/`)
}
...
const settingsLinks = computed<LinkItem[]>(() => [
  ...
  { label: '成员权限', icon: navMember, to: `/projects/${projectId.value}/settings/members` },
  { label: '角色权限', icon: navMember, to: `/projects/${projectId.value}/settings/roles` },
...
onMounted(() => {
  ...
  try {
    const rawAsset = localStorage.getItem(`${localStorageKey}.group.asset.open`)
    const rawExec = localStorage.getItem(`${localStorageKey}.group.execution.open`)
    isGroupOpen.value = {
      asset: rawAsset == null ? true : rawAsset === '1' || rawAsset === 'true',
      execution: rawExec == null ? true : rawExec === '1' || rawExec === 'true'
    }
  } catch {}
  applySidebarWidth()
})
...
<template>
  <aside ref="sidebarRef" class="flex h-full min-h-screen flex-col bg-[#FAFAFA] border-r border-[#E5E5E5] transition-[width] duration-300 ease-in-out overflow-hidden">
    <div class="relative flex h-[66.17px] items-center gap-[10px] border-b border-[#E5E5E5] pl-[16px]" :class="isCollapsed ? 'justify-center pl-0' : ''">
      ...
      <button
        type="button"
        class="absolute right-[10px] top-1/2 -translate-y-1/2 flex h-[26px] w-[26px] items-center justify-center rounded-[8px] hover:bg-black/5"
      ...
      >
        <svg
          ...
          class="h-[16px] w-[16px] text-[rgba(10,10,10,0.7)]"
        >
    ...
    <nav class="flex-1 overflow-y-auto overflow-x-hidden pb-[12px]">
      ...
      <div v-for="group in groups" :key="group.label" class="px-[8px] pt-[2px]">
        <div class="flex w-full flex-col">
          <button
            type="button"
            class="flex h-[36px] items-center gap-[8px] rounded-[10px] px-[12px]"
            :class="isCollapsed ? 'justify-center px-0' : ''"
            @click="toggleGroupExpanded(group.label)"
          >
            ...
            <img
              ...
              :src="group.chevron"
              alt=""
              class="h-[14px] w-[14px] transition-transform duration-300"
          ...
          <div
            v-if="!isCollapsed"
            class="ml-[16px] mt-[4px] flex w-[174.67px] flex-col gap-[2px] overflow-hidden transition-[max-height,opacity] duration-300 ease-in-out"
            :style="groupChildrenStyle(group.label)"
          >
            <button
      ...
      <div class="mx-[8px] mt-[10px] border-t border-[#E5E5E5] pt-[10px]">
        ...
        <div class="mt-[6px] flex flex-col gap-[2px]">
          <button
            ...
            type="button"
            class="flex h-[36px] w-full items-center gap-[8px] rounded-[10px] pl-[12px]"
            :class="[isSettingsLinkActive(item) ? 'bg-[#155DFC]' : '', isCollapsed ? 'justify-center pl-0' : '']"
            @click="navigateTo(item)"
          >
            <img :src="item.icon" alt="" class="h-[16px] w-[16px] flex-shrink-0" :class="isSettingsLinkActive(item) ? 'filter brightness-0 invert' : ''" />
            <span v-if="!isCollapsed" class="text-[14px] leading-[20px]" :class="isSettingsLinkActive(item) ? 'text-white' : 'text-[rgba(10,10,10,0.7)]'">{{ item.label }}</span>
          </button>
        </div>
      </div>
    </nav>
