...
import type { UiTestReportListItem } from '@/lib/aiTestingPlatformApi'

const modelValue = defineModel<string>({ required: true })

defineProps<{
  items: UiTestReportListItem[]
  loading?: boolean
}>()
...
<template>
  <div class="flex flex-wrap items-center gap-[12px]">
    <div class="relative h-[32px] w-full max-w-[420px]">
      <select
        v-model="modelValue"
      ...
      >
        <option v-for="item in items" :key="item.runId" :value="item.runId">
          {{ item.pageId }} · {{ item.assertLevel }} · {{ new Date(item.createdAt * 1000).toLocaleString() }}
        </option>
      </select>
